...
public class MazeAssemblerTest {
    ...
    public void testAssembleMazeCorrectlyCreatesPassagesAndWalls() {
        ...
        mazeEdges.add(new Edge(new Cell(0, 1), new Cell(1, 1))); // Passage between (1,0) and (1,1)

        maze.assembleMaze(outputMaze, mazeEdges, 2, 2);
    ...
    public void testAssembleMazeBordersAlwaysHaveEdges() {
        ...
        for (int i = 3; i < outputMaze.size() - 2; i += 2) { // Iterate through each row containing vertical boundaries
            assertThat(outputMaze.get(i)).isEqualTo("|    |    |    |    |    |");
        }
        ...
        assertThat(topBorder).isEqualTo("N++++N++++N++++N++++N++++N");
        assertThat(bottomBorder).isEqualTo("N++++N++++N++++N++++N++++N");
    }
}
